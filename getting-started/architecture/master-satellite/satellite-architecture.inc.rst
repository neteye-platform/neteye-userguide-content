...
each Satellite and client in its zone (where applicable).
...
|ne| implements secure communication between Satellites and Master;
each Satellite is responsible for handling a set of hosts.  Different `agents` can
also be installed on the hosts; these software components perform specific
tasks locally on the host and are connected to the Satellite.
...
|ne| Satellites allow to reduce the load on Master and also the
...
The second scenario sees |ne| Satellites operate in an isolated
...
of the box - can be very helpful in executing many operations to
...
Satellites use the NATS Server, the default message broker in |ne|, to transport
...
The services that need to run on the |ne| Satellite Nodes are managed by a dedicated `systemd target`
named ``neteye-satellite.target``, which takes care of starting and stopping the services
of the Satellite when needed.

Various services are configured and activated out of the box on |ne| Satellites:
among others, the Tornado Collectors and those provided by Icinga 2.

For the complete list of the services enabled on a |ne| Satellite, on the Satellite you can run:
