...
|ne|. The system requirement are intended for a Single Node, a
...
requirements. Moreover, also disk space may become an issue when the
amount of logs produced by a |ne| installation, by its monitored
objects, or by both is large.
...
Node**, to allow for updates and licence verification:

.. csv-table::
   :header: "Domain", "Port", "Intended Use"

   "repo.wuerth-phoenix.com", "443 TCP", "Würth IT Italy repository for NetEye update/upgrade"
   "api.neteye.cloud", "443 TCP", "Würth IT Italy API used during NetEye update/upgrade"
...
.. csv-table::
   ...
   "bitbucket.org", "443 TCP", "Download customized script and plugin (often used by NetEye consultants)"
...
environments. For each one are listed some options that need to he
...
NetEye 4 is using LDAP protocol for the purpose of binding users within
Active Directory, OpenLDAP, to a centralized account. Hence, NetEye adheres
to the `LDAP Standards Track <https://datatracker.ietf.org/doc/html/rfc4511>`__.

In order to log in to NetEye 4 with a centralized account, create an
...
from NetEye 4 to the LDAP system directory.
...
* To send notifications via SMTP you need an SMTP Relay Server, which should be reachable by NetEye Nodes as described
...
* In order to send SMS messages, unset the :ref:`PIN on your SIM card
  ...
  * To handle SMS we provide two types of modem:
    ...
    * :ref:`SMS Gateway connected via serial bus <sms-gateway-moxa>`
      (contact your NetEye 4's consultant for further information)
