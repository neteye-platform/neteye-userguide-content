...
#. Run the following command to download and install the latest Alyvix master release along
   ...
   If necessary, you can install Alyvix through a proxy by adding the following parameter, however it is highly recommended to use a credential free command:
...
Once completed, you can also manually remove the :file:`C:\\Python314\\` directory (or whichever
