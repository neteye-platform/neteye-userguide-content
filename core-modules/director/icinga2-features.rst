...
also ship some activated features, in specific the `livestatus`, `influxdb`, `ido-mysql` and
`icingadb`. As a special feature that will only be pre-configured, but not automatically enabled,
we have the `neteye_datastreamwriter`.

..
  Todo: Document also the influxdb and icingadb feature. Remove the ido-mysql in the 4.47
