...
opened on Corporate Network and/or Private (Heartbeat) Network
to allow |ne| to operate correctly. These requirements apply on
...
a Corporate Network in order to allow its communication with the |ne|.

Additionally, the communication between the |ne| and Corporate Network
...
.. csv-table:: TCP/UDP Ports Requirements for System and Management
   ...
   used to manage NetEye should reach the Cluster Virtual IP via
   HTTP/S. Satellites use those ports to receive data from agents."
   "TCP 22","**Node SSH Console**, Inbound", "Master, Satellite", "Systems used to manage deep NetEye
   ...
   If the Icinga2 notification feature is enabled on a Satellite as well, same ports should be opened on the latter."
   ...
   your Active Directory user accounts the ability to access NetEye,
   each node must be able to contact at least one DC on both ports 636
   (LDAP) and 3269 (Global Catalog) encrypted over SSL.
   To allow your LDAP user account the ability to access NetEye, each
   ...
   the NetEye Master NATS Leaf Node port in order to send data generated on the Satellite to the Master."
   "TCP 4222", "**NATS Server**", "Master (Inbound), Satellite", "In order to send data from a NATS Client (e.g. a Telegraf)
   directly to NetEye, port 4222 should be opened (NATS TCP port)."
...
NetEye) and agent deployment, while the *Physical Node's IP* is used
for **active monitoring** (i.e., requests from NetEye to devices).

We distinguish the following types of monitoring:

* **Active monitoring** through ``ICMP`` consisting of direct ``ICMP``
  requests from NetEye to monitored devices
...
ports. Moreover, also the operating system installed on the devices to
be monitored influences the ports to be opened; all are reported in
...
Depending on the services enabled on the cluster, take into account
the following:
...
* You may need to contact your NetEye 4 consultant for the following
  requirements:
  ...
  * Allow connections between NetEye 4 and all VLANs/Subnets involved
    in monitoring
...
Individual NetEye modules may have their own specific requirements that
will need to be taken into consideration if a particular Module is to be enabled.
...
.. csv-table:: Alyvix Port Requirements
   ...
   "TCP 4222", "While TCP 4222 grants Inbound connection from Alyvix Service to NetEye,
   TCP 443 should also be opened to allow Outbound connection from NetEye to Alyvix Service"
...
NetEye cluster: DRBD, PCS, and Elasticsearch. Like Elastic-only nodes,
