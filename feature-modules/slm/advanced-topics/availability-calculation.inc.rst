.. warning::
   The availability calculation described in this section assumes the "IcingaDB Historical Data"
...
after *time_range.from* (see :ref:`slmd-ido-retention-interaction` for details).
...
.. warning:: The Icinga2 IDO retention policy settings may affect the SLM availability
   Reports. Please see :ref:`Interaction with IDO retention policy <slmd-ido-retention-interaction>`
   for more details.
...
.. _slmd-ido-retention-interaction:

Interaction with IcingaDB SLA retention policy
++++++++++++++++++++++++++++++++++++++++++++++

The IcingaDB SLA retention policy settings may affect the SLM availability
Reports. This happens because the availability calculations of the SLM module relies on
the state change events present in the IcingaDB SLA tables ``sla_history_state`` and ``sla_history_downtime``
...
on the time it was generated, since the IcingaDB retention policy continuously
removes events older than the defined age in days. For this reason, please configure the
days in the IcingaDB SLA retention policy, with a value sufficient to avoid impacting
...
To overcome some issues deriving from the deletion of old Icinga2 events, the SLM
availability calculation uses two additional pieces of information present in IcingaDB,
...
Each event present in the IcingaDB SLA database has a **previous_hard_state** property, which
...
To help you understand how the SLM availability computation works together with the IDO retention
