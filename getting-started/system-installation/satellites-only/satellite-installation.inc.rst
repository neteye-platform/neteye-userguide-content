...
Satellites, then guide you through the steps needed to configure a new
|ne| Satellite.

For further information about |ne| Satellites, refer to the Section :ref:`master-satellite-arch`.
...
met for Satellites to operate properly.

* Both the Master and the Satellite must run the **same NetEye
  version**
...
* If you are in a |ne| cluster environment, check that all resources
...
* **icinga2_wait_for_satellite_connection**: if set to *false* the
  ...
  parameter is optional and the default value is *true*.
