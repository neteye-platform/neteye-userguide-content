...
Some software installed on the |ne| Nodes needs to have access to resources from the Internet.
In certain environments, however, the |ne| Nodes do not have direct Internet access and instead need
to pass through a proxy which forwards the requests to the wider web. In these cases you
need to configure some parts of |ne| so that the software can access the proxy.

Assume your |ne| Nodes need to pass through a proxy which has the settings below:
...
information on the :file:`dnf.conf` options):
...
If you need to install additional Elasticsearch plugins via the :command:`elasticsearch-plugin` utility, or
