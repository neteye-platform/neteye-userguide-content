...
by collecting them, processing them to detect anomalies and threats, and finally visualise them.
...
produced by NetEye, various services running on it and the logs collected from external systems with the help of Beats and Elastic Agents.

Typical components of a SIEM solution include:

* a log collector, which can be multiple software that concur to
  receive log files and convert them to a given format
* a storage facility, typically a (distributed) database
* a visualisation engine, to create dashboards and reports
* some kind of time-stamping solution, to provide data
  unchangeability, useful for log auditing and compliance to laws and regulations
...
In the similar way NetEye can receive data from *Beats* installed on monitored hosts.
...
`Logstash <https://www.elastic.co/logstash>`__ is an open source server-side data processing pipeline that ingests data from
multiple sources, transforms it, and then sends to a preferable storage.

In the context of SIEM solution, Logstash is responsible for the collection of logs,
...
After the data are signed and hence integrated into a secure blockchain, they are sent to Elasticsearch.
