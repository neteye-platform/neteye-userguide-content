...
:ref:`cluster-services-installation`,
...
#. Run the cluster setup command :command:`neteye cluster install` to install a
   basic Corosync/Pacemaker cluster with a floating clusterIP. In case of any
...
This section describes the procedures you can use to configure, test, and manage the fence devices in a cluster.
Fencing is useful when occur that a node is unresponsive and may still be accessing data.
...
#. Enable fencing
   ...
   .. _cluster-services-installation:
...
When installing a feature module, it is necessary to run all the related services on a |ne| Cluster.
In order to do that, adjust all necessary options, including IPs, ports, DRBD devices, sizes, in the
various :file:`*.conf.tpl` files found in directory
:file:`/usr/share/neteye/cluster/templates/`.
...
* **ip_pre**, the prefix of the IP (e.g., *192.168.1* for
  *192.168.1.0/24*), which will be used to generate the virtual IP
  for the resource

* **cidr_netmask**, the CIDR of the internal subnet used by IP
  resources (e.g., *24* for *192.168.1.0/24*).

* **size**, the volume of the storage assigned to the service in MB. You can specify the volume,
  otherwise default values will be applied.

Run the :command:`cluster_service_setup.pl` script on **each**
:file:`*.conf.tpl` file starting from
:file:`Services-core.conf.tpl`::

         # cd /usr/share/neteye/scripts/cluster
         # ./cluster_service_setup.pl -c Services-core.conf.tpl

The :command:`cluster_service_setup.pl` script is designed to report
...
configured resource template from :file:`Services.conf` and re-run
that command. Then you should re-execute the
:command:`cluster_service_setup.pl` script in
order to finalize the configuration.

.. _non-pcs-managed-services:
...
PCS-managed services, you need to define necessary options, which may include
IPs, ports, Volume groups, sizes, in a :file:`.yaml` file
...
services configuration files in directory :file:`/etc/neteye-services.d/<module_name>`,
...
name, removing the ``.tpl`` extension.

.. warning::
    Files should be present on all neteye nodes before proceeding with the installation.

After all the service configuration files are in place, you can proceed
to the next step of the installation procedure.
