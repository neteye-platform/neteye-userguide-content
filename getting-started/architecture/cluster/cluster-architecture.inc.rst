...
The clustering service of |ne| 4 is based on the RedHat 8 High
...
Voting-only nodes. A |ne| cluster is a **failover** cluster at service
...
these cases, services are moved to another node if necessary.
...
* A networking issue (failure of a network interface or in the
  connectivity) which prevents a node from communicating with the other
  nodes
* A hardware or software issue that freezes or blocks a node
...
:numref:`figure-neteye-cluster` illustrates the structure of a |ne| cluster and how it achieves High Availability.
...
.. topic:: Cluster Failure and Voting-only Nodes
   ...
   A cluster composed of *N* nodes requires that at least *N/2 + 1*
   **operative** nodes be online to operate properly (the
   `quorum`). For example, a cluster composed of 3 nodes needs in
   theory at least 2.5 nodes to operate properly. This means that
   whenever one of the three nodes goes offline, the cluster no
   longer works because the quorum cannot be reached. Adding a
   Voting-only node ensures that the cluster remains operational
   in these cases, because as soon as a node goes offline, it
   will count as a regular node.

.. seealso:: Voting-only nodes and their use are described in great
   detail in a |ne| blog post:
...
rather than Red Hat HA Clustering. |ne| will also take advantage of
their inbuilt load balancing capabilities.

Icinga 2 Cluster
   An Icinga 2 cluster is composed of one **master** instance holding
   configuration files and a variable number of satellites and agents.
...
Elasticsearch
   ...
   which nodes can form a quorum (note that all |ne| cluster nodes
   ...
   .. seealso:: Elastic clusters and Elastic-only nodes are described
      in more detail in the :ref:`elastic-cluster` section.

Kibana
   The Kibana service runs on all the nodes that are
...
Galera
   ...
   .. warning::
      ...
      - When starting or restarting a Galera node, the systemctl command will
        ...
        synchronization process can take more or less time depending on
...
|ne|'s clustering functionalities:
...
“Local” |ne| services running *simultaneously* on *each* |ne| node
...
is managed exactly like the :ref:`Single Node neteye target
...
.. note:: In order to keep data and configuration synchronisation,
   ...
   They consist of operating nodes and Voting-only nodes, while Elasticsearch nodes are
